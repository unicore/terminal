...
div
  div(id="history")
    p(style="letter-spacing: 2px; font-size: 10px;").q-mb-md.text-center ИСТОРИЯ ОБМЕНА
    ...
    div(v-if="Object.keys(history).length > 0" v-for="dhist in Object.values(history)" v-bind:key="dhist.id").full-width.q-pt-md
      ...
      span(style="font-size: 10px; letter-spacing: 2.1px;").full-width 
        ...
        span(v-if="dhist.action=='deposit'") купил фракций на: 
        span(v-if="dhist.action=='burn'") обменял {{host_obj.symbol}} на: 
        span(v-if="dhist.action !='burn' && dhist.action !='deposit'") вернул фракции и получил компенсацию: 
        // p {{dhist}}
